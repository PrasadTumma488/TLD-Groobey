...
add column if not exists groobey_code text;

create unique index if not exists profiles_groobey_code_key
on public.profiles (groobey_code)
where groobey_code is not null;
